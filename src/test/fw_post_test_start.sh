...
#
# NOTE: All of these tests "fail" because none of these ports ever reach the FORWARD chain in the first place.
# To cause this to pass, we'd first have to explicitly try to allow them (e.g. through user-defined allowed ports),
# run the generated tests, and THEN check these results.
#
# We decided not to do that; instead, the user should look at the output from external_tests.sh to see that these
# tests do actually work.
#
